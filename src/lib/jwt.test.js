...
describe('Cryptographic JWT & Zero Trust Cookie Engine', () => {
  it('signs and verifies valid JWT tokens correctly', async () => {
  ...
  it('rejects tampered JWT tokens', async () => {
    const token = await signJwt({ sub: 'USER-123', role: 'PRINCIPAL' });
    ...
    expect(await verifyJwt(tamperedToken)).toBeNull();
  });

  it('rejects expired JWT tokens', async () => {
    // Expired -10 seconds ago
    const token = await signJwt({ sub: 'USER-123' }, -10);
    expect(await verifyJwt(token)).toBeNull();
  });

  it('creates OWASP-compliant HttpOnly SameSite=Strict Set-Cookie headers', async () => {
